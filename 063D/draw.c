		...
		double r, int step)
{
	...
	for (i = 0; i < sphere_points->lastcol; i += 2) {
		add_edge(edges, sphere_points->m[0][i], sphere_points->m[1][i], sphere_points->m[2][i],
				sphere_points->m[0][i] + 1, sphere_points->m[1][i] + 1, sphere_points->m[2][i] + 1);
	}
		...
		double r, int step)
{
	...
	for (p = 0; p <= step; p++) {
		for (t = 0; t <= step; t++) {
			x = r * cos(t * M_PI / step * 10) + cx;
			y = r * sin(t * M_PI / step * 10) * cos(p * 2 * M_PI / step * 10) + cy;
			z = -r * sin(t * M_PI / step * 10) * sin(p * 2 * M_PI / step * 10) + cz;
		...
		double r1, double r2, int step)
{
	...
	for (i = 0; i < torus_points->lastcol; i += 2) {
		add_edge(edges, torus_points->m[0][i], torus_points->m[1][i], torus_points->m[2][i], torus_points->m[0][i + 1], torus_points->m[1][i + 1], torus_points->m[2][i + 1]);
	}
		...
		double r1, double r2, int step)
{
	...
	for (p = 0; p <= step; p++) {
		for (t = 0; t <= step; t++) {
			x = -(r2 + r1 * cos(t * 2 * M_PI / step)) * cos(p * 2 * M_PI / step) + cx;
			y = -r1 * sin(t * 2 * M_PI / step) + cy;
			z = -(r1 * cos(t * 2 * M_PI / step) + r2) * sin(p * 2 * M_PI / step) + cz;
