		...
		screen s)
{
	...
	while (fgets(line, 255, f) != NULL) {
		...
		if (!strcmp(line, "line")) {
			...
			//get line with coordinates
			fgets(line, 255, f);

			//store coordinates
			sscanf(line, "%lf %lf %lf %lf %lf %lf", &x0, &y0, &z0, &x1, &y1, &z1);

			//add coordinates to edges matrix
		...
		//translate
			...
			//get line with translate values
			fgets(line, 255, f);

			//store translating factors
			double tx, ty, tz;
