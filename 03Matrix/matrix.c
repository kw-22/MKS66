...
void print_matrix(struct matrix *m)
{
	int r, c;
	for (r = 0; r < m->rows; r++) {
		for (c = 0; c < m->lastcol; c++) printf("%6.2lf ", m->m[r][c]);
		printf("\n");
	}
	printf("\n");
}
...
void ident(struct matrix *m)
{
	...
	for (r = 0; r < m->rows; r++)
		for (c = 0; c < m->lastcol; c++)
			m->m[r][c] = (r == c) ? 1 : 0; //assign on diagonal
}
...
{
	if (a->lastcol != b->rows) return;
	else {
		struct matrix *tmp = new_matrix(a->rows, b->lastcol); //create matrix with proper dimensions
		int r, c, i;
		for (r = 0; r < a->rows; r++)
			for (c = 0; c < b->lastcol; c++) {
				tmp->m[r][c] = 0;
				for (i = 0; i < b->rows; i++) tmp->m[r][c] += a->m[r][i] * b->m[i][c]; //compute entry using dot product
			}
		copy_matrix(tmp, b);
		free_matrix(tmp);
	}
